...
const insertYacht = async (newYacht) => {
    const insert = await pool.query(
        ...
        [
            ...
            newYacht.beam_wl,
            newYacht.draft,
            newYacht.displacement,
            newYacht.centre_of_buoyancy,
            newYacht.prismatic_coefficient,
            newYacht.velocity,
            newYacht.resistance,
        ]
    )
...
const selectYachts = async () => {
    const allYachts = await pool.query('SELECT * FROM yacht ORDER BY id DESC')

    return allYachts.rows
}

const selectYacht = async (id) => {
    const result = await pool.query('SELECT * FROM yacht WHERE id = $1', [id])
...
const updateYacht = async (id, data) => {
    const result = await pool.query(
        ...
        [
            ...
            data.draft,
            data.displacement,
            data.centre_of_buoyancy,
            data.prismatic_coefficient,
            data.velocity,
            data.resistance,
            id,
        ]
...
const deleteYacht = async (id) => {
    const result = await pool.query('DELETE FROM yacht WHERE id = $1', [id])
    if (!result) {
        throw createCustomError(`No yacht with id: ${id}`, 404)
    }
}
